...
while True:
    user_choice = input("Escolha R(Pedra)/T(Tesoura)/P(Papel) ou Q para sair)").lower() #todos os caracteres sejam transformados em sua versão minuscula

    if user_choice == 'q':
        break

    if user_choice not in options: # verifica a condifional da esquerda para a direita
        continue

    computer_choice = random.randint(0, 2)  # 0 : R, 1 : T, 2 : P
